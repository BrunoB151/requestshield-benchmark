...
write_files:
  # 0600 because BENCH_API_KEY is a secret. Read by systemd (root) via EnvironmentFile;
  # debug with `sudo cat /etc/rsedge-bench/env`.
...
runcmd:
  ...
  # 2. Clone the benchmark repo. Public over HTTPS — anonymous clone.
  ...
  #    The GitHub mono-repo nests the benchmark under projects/github-requestshield-benchmark/;
  #    we clone to /opt/rsedge-bench-src and symlink that subdir to the stable
  #    path the systemd unit expects. No `|| true` — a broken clone must fail loudly.
  - git clone --depth=1 --branch ${benchmark_repo_ref} ${benchmark_repo} /opt/rsedge-bench-src
  - ln -sfn /opt/rsedge-bench-src/projects/github-requestshield-benchmark /opt/requestshield-benchmark
